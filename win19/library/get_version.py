...
---
module: get_version
short_description: Get Windows OS version and return changed flag if below min
description:
  - Checks Windows OS version and returns the version data in JSON
  - Checks the min version against the passed "major,minor,build" parameters
  - Returns "changed" if the remote system's OS version is below a minimum
  - The "changed" flag is to notify operations or to trigger another task
options:
  major:
    description:
